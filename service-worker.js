const CACHE_NAME = 'hapcapex-v40-0-98-filter-engine-stability-20260915';
const APP_SHELL = [
  './',
  './index.html',
  './controle-capex.html',
...
async function decorateDashboardCoreResponse(response) {
  ...
  return responseWithText(response,patched.text,'application/javascript; charset=utf-8',{
    'x-hapcapex-functional':'v40.0.98','x-hapcapex-excel-filters':patched.applied?'curve-active':'not-applied'
  });
}
...
async function decorateControlManagerialResponse(response) {
  ...
  return responseWithText(response, excelPatch.text, 'application/javascript; charset=utf-8', {
    'x-hapcapex-functional': 'v40.0.98',
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      if (cached) {
        if (cached.headers.get('x-hapcapex-functional') === 'v40.0.98') return cached;
