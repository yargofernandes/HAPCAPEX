const CACHE_NAME = 'hapcapex-v40-0-102-idempotent-filter-runtime-20260915';
const APP_SHELL = [
  './',
  './index.html',
  './controle-capex.html',
...
const CONTROL_GOVERNANCE_TAG = '<script src="./v37-control-governance.js?v=40.0.102"></script>';
...
function stripInjectedFilterRuntimesV40102(text) {
  if (!text) return text;
...
async function decorateDashboardCoreResponse(response) {
  ...
  return responseWithText(response,patched.text,'application/javascript; charset=utf-8',{
    'x-hapcapex-functional':'v40.0.102','x-hapcapex-excel-filters':patched.applied?'curve-active':'not-applied'
  });
}
...
async function decorateControlManagerialResponse(response) {
  ...
  return responseWithText(response, excelPatch.text, 'application/javascript; charset=utf-8', {
    'x-hapcapex-functional': 'v40.0.102',
...
function patchControlGovernanceFilterUiV40102(source) {
  if (!source) return { text: source, applied: false };
  if (source.includes('HAP_V40102_EXCEL_FILTER_AUTHORITY')) return { text: source, applied: true };
  let text = source;
  let applied = false;
  ...
  if (text.includes(activeFilterLine)) {
    text = text.replace(
      activeFilterLine,
      "    if (state.v37) state.v37.pendencyFilter = 'all';\n    const activeFilter = 'all'; // HAP_V40102_EXCEL_FILTER_AUTHORITY"
    );
    applied = true;
  }
  const decorateCall = '    decorateCapexPendencyFilterV378();';
  if (text.includes(decorateCall)) {
    text = text.replace(decorateCall, '    // HAP_V40102_EXCEL_FILTER_AUTHORITY — filtro horizontal de pendências removido; use os filtros de coluna.');
    applied = true;
  }
  return { text, applied };
}
...
async function decorateControlGovernanceResponse(response) {
  ...
  const filterPatch = patchControlGovernanceFilterUiV40102(auditPatch.text);
...
async function decorateHtmlResponse(response, url) {
  ...
  let text = await response.text();
  text = stripInjectedFilterRuntimesV40102(text);
  text = addSri(text);
