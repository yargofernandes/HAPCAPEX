/* HAPCAPEX V31 — Consumo Não Planejado editável diretamente pela linha da tabela */
(() => {
  'use strict';
  ...
  function injectStyles() {
    ...
    style.id = 'v30Styles';
    style.textContent = `
      .v31-nonplanned-clickable{cursor:pointer!important;transition:filter .12s ease,box-shadow .12s ease}
      .v31-nonplanned-clickable:hover{filter:brightness(.97);box-shadow:inset 3px 0 0 #e07020}
  ...
  function removeLegacyButtons() {
    ['nonPlannedEditBtn','reclassifyRealizedBtn','mobileNonPlannedEditBtn','mobileReclassifyRealizedBtn'].forEach(id => byId(id)?.remove());
  }

  function bindNonPlannedRows() {
    if (!isAdmin()) return;
    document.querySelectorAll('tr').forEach(row => {
      if (row.closest('#nonPlannedEditModal, #reclassifyRealizedModal')) return;
      const normalized = String(row.textContent || '')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/\s+/g, ' ').trim().toUpperCase();
      if (!normalized.includes('CONSUMO NAO PLANEJADO') && !normalized.includes('OBRAS NAO PLANEJADAS')) return;
      row.classList.add('v31-nonplanned-clickable');
      row.title = 'Clique para editar o Consumo Não Planejado';
      if (row.dataset.v31NonplannedBound === '1') return;
      row.dataset.v31NonplannedBound = '1';
      row.addEventListener('click', event => {
        if (event.target.closest('button,a,input,select,textarea,label')) return;
        event.preventDefault();
        event.stopPropagation();
        openNonPlannedEditor();
      }, true);
    });
  }

  function observeNonPlannedRows() {
    removeLegacyButtons();
    bindNonPlannedRows();
    const root = byId('dashboardRoot') || document.body;
    const observer = new MutationObserver(() => bindNonPlannedRows());
    observer.observe(root, { childList:true, subtree:true });
    [250,700,1400,2500].forEach(delay => setTimeout(bindNonPlannedRows, delay));
  }
  ...
  function injectModals() {
    ...
    document.body.insertAdjacentHTML('beforeend', `
      <div id="nonPlannedEditModal" class="admin-modal admin-only"><form id="nonPlannedEditForm" class="admin-box v30-box">
        ...
        <div class="admin-actions"><button id="v30CancelNonPlanned" type="button">Cancelar</button><button id="v31ReclassFromNonPlanned" type="button">↔ Reclassificar consumo</button><button class="save" type="submit">Salvar e recalcular HAPCAPEX</button></div>
      </form></div>
    ...
    byId('v31ReclassFromNonPlanned').addEventListener('click', () => { closeV30Modal('nonPlannedEditModal'); openReclassification(); });
  ...
  async function saveReclassification(event) {
    ...
    submit.disabled = true; status.className = 'v30-status'; status.textContent = 'Criando backup de segurança...';
    try {
      await backup(`Antes da reclassificação de realizado — ${V30_LABELS[month]} — V31`);
  ...
  function initialize() {
    ...
    injectStyles(); injectModals(); observeNonPlannedRows();
    return true;
  }
