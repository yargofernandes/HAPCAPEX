...
function computeFlowRaw(obra) {
  ...
  const rule = obra._flowRule || (obra._baselineFlow ? 'historical_baseline' : 'standard_15_75_10');
  const params = obra._flowRuleParams || {};

  if (rule === 'oper_realized_plus_balance_dec') {
    ...
    if (saldoResidual > FLOW_TOLERANCE) {
      ...
      allocationKeys.forEach((key, index) => {
        flow[key] = Number(flow[key] || 0) + (parcelaCentavos + (index < restoCentavos ? 1 : 0)) / 100;
      });
    }
    return flow;
  }

  if (rule === 'historical_baseline') {
  ...
  if (rule === 'contingency_full' || rule === 'realized_equals_forecast') {
    MONTHS_REAL.forEach(mk => { flow[mk] = Number(obra[mk+'_real'] || 0); });
    return flow;
  }

  if (rule === 'contingency_partial_hapfor') {
    MONTHS_REAL.forEach(mk => { flow[mk] = Number(obra[mk+'_real'] || 0); });
    const saldo = Math.max(0, Number(obra.capex || 0) - flowSum(flow));
    const reportingIndex = currentReportingIndex();
    const futureKeys = ['jul','ago','set','out'].filter(key => MONTHS.findIndex(m => m.key === key) > reportingIndex);
    if (saldo > 0) allocateByWeights(flow, futureKeys.length ? futureKeys : [MONTHS[reportingIndex]?.key || 'jul'], saldo, Object.fromEntries((futureKeys.length ? futureKeys : [MONTHS[reportingIndex]?.key || 'jul']).map(key => [key,1])), true);
    return flow;
  }

  if (rule === 'contingency_partial_standard') {
    MONTHS_REAL.forEach(mk => { flow[mk] = Number(obra[mk+'_real'] || 0); });
    const saldo = Math.max(0, Number(obra.capex || 0) - flowSum(flow));
    if (saldo > 0) {
      const reportingIndex = currentReportingIndex();
      const futureKeys = MONTHS.slice(reportingIndex + 1).map(month => month.key);
      allocateByWeights(flow, futureKeys.length ? futureKeys : [MONTHS[reportingIndex]?.key || 'jul'], saldo, standardScheduleWeights(obra), true);
    }
    return flow;
  }

  if (rule === 'single_payment') {
    const key = String(params.month || 'jul');
    if (Object.prototype.hasOwnProperty.call(flow, key)) flow[key] = Number(obra.capex || 0);
    return flow;
  }

  if (rule === 'close_by_balance') {
    const reportingIndex = currentReportingIndex();
    const closedKeys = MONTHS.slice(0, reportingIndex).map(m => m.key);
    closedKeys.forEach(key => { flow[key] = Number(obra[key+'_real'] || 0); });
    const used = flowSum(flow);
    const closingKey = MONTHS[Math.min(reportingIndex, MONTHS.length - 1)]?.key || 'dez';
    flow[closingKey] = Math.max(0, Number(obra.capex || 0) - used);
    return flow;
  }

  const capex = Number(obra.capex || 0);
  if (!capex) return flow;

  if (rule === 'linear_year') {
    const monthly = capex / 12;
    MONTHS.slice(0,12).forEach(m => { flow[m.key] = monthly; });
    return flow;
  }
  ...
  // Para obras novas, não reescrevemos meses anteriores ao primeiro mês em que
  // a obra entrou no sistema. Obras replanejadas manualmente mantêm esta proteção.
  if (!obra._isOriginalBaseline && window.HAP_DATA.reportingMonthKey) {
    ...
    const reportYM = reportIndex >= 0 && reportIndex < 12 ? `2026-${String(reportIndex + 1).padStart(2,'0')}` : null;
    if (reportYM) {
      if (ymFim < reportYM) {
        const reportKey = window.HAP_DATA.reportingMonthKey;
        if (flow[reportKey] !== undefined) flow[reportKey] = capex;
        return flow;
      }
  ...
  const startPct = rule === 'rio_5_85_10' ? 0.05 : 0.15;
  const execPct  = rule === 'rio_5_85_10' ? 0.85 : 0.75;
  const retentionPct = 0.10;
  const startValue = capex * startPct;
  const executionValue = capex * execPct;
  const retentionValue = capex * retentionPct;

  const execMonths = [];
  let cur = addMonthsYM(ymInicio, 1);
  while (cur <= ymFim) { execMonths.push(cur); cur = addMonthsYM(cur, 1); }
  const execMonthly = execMonths.length ? executionValue / execMonths.length : executionValue;
  const startKey = ymToKey(ymInicio);
  if (startKey) flow[startKey] += startValue + (execMonths.length ? 0 : executionValue);
  execMonths.forEach(ym => { const key = ymToKey(ym); if (key) flow[key] += execMonthly; });
  const r1 = ymToKey(addMonthsYM(ymFim,1)); if (r1) flow[r1] += retentionValue / 2;
  const r2 = ymToKey(addMonthsYM(ymFim,2)); if (r2) flow[r2] += retentionValue / 2;
  return flow;
}

function computeFlow(obra) {
  let flow = computeFlowRaw(obra);
  if (obra._flowRule === 'oper_realized_plus_balance_dec' || obra._flowRule === 'realized_equals_forecast') return flow;
  if (obra._flowRule === 'historical_baseline' && obra._baselineFlow && typeof obra._baselineFlow === 'object') {
    flow = rebaseBaselineFlowToCurrentCapex(obra, flow);
  }
  // Regras que intencionalmente não possuem saldo futuro só são reconciliadas
  // quando o CAPEX ainda não foi completamente consumido para manter a integridade
  // gerencial do portfólio. A diferença é posicionada no primeiro mês aberto.
...
window.HAP_RUNTIME_OBRAS = obras;
...
// A regra "Previsto = Realizado" é, por definição, uma exceção à igualdade
// fluxo total = CAPEX enquanto houver saldo não realizado. Por isso, a
// conciliação técnica usa como alvo o próprio fluxo para essa regra e o CAPEX
// para todas as demais regras reconciliáveis.
const ALVO_INTEGRIDADE_FLUXO = obras.reduce((s, obra) => s + (obra._flowRule === 'realized_equals_forecast' ? flowSum(obra.flow) : Number(obra.capex || 0)), 0);
const OBRAS_OPER = obras.filter(o => o._flowRule === 'oper_realized_plus_balance_dec');
...
const DIFERENCA_INTEGRIDADE_FLUXO = ALVO_INTEGRIDADE_FLUXO - PREVISTO_OBRAS_TABELA;
window.HAP_FINANCIAL_CHECK = {
  ...
  alvoIntegridadeFluxo: ALVO_INTEGRIDADE_FLUXO,
...
if (Math.abs(DIFERENCA_INTEGRIDADE_FLUXO) > 0.01) {
  ...
  if (obrasContainer) {
    ...
    warning.innerHTML = '<strong>⚠️ Alerta de integridade financeira:</strong> o fluxo previsto não está conciliado com as regras financeiras vigentes das obras. Não utilize os totais antes de revisar a importação.';
...
function openPanel(idx) {
  ...
  const ruleDef = (window.HAP_DATA?.flowRules || []).find(rule => rule.code === o._flowRule);
  const ruleTag = ruleDef ? ` · <span class="panel-rule-tag">${ruleDef.name}</span>` : '';
  document.getElementById('panel-tipo').innerHTML = `${tipoEmojis[o.tipologia] || '📌'} ${o.tipologia}${panelStatus}${ruleTag}`;
  document.getElementById('panel-nome').textContent = o.nome;
  const editBtn = document.getElementById('editWorkBtn');
  if (editBtn) {
    const canEdit = window.HAP_DATA?.currentProfile?.role === 'admin';
    editBtn.hidden = !canEdit;
    editBtn.onclick = canEdit ? () => window.openWorkEditor?.(o._id) : null;
  }
...
applyFilter();
applyManFilter();
setTimeout(() => window.HAP_NOTIFICATIONS?.syncDerived?.(obras), 0);